  ...
  return (
    <div className="relative min-h-screen bg-[#f8f9fd] p-8 flex items-center justify-center overflow-hidden">
      ...
      <div className="w-full max-w-4xl bg-white border border-[#575B91] rounded-xl shadow-xl p-6 z-10">
        <h2 className="text-2xl font-bold text-[#575B91] mb-4  flex flex-row items-center sour gap-2 uppercase">
          <span className="sour uppercase flex flex-row items-center">
        ...
        <div className="mockup-code bg-[#f0f1fa] text-[#2d2e4c] h-80 overflow-y-auto mb-4 rounded-md">
          {output.length === 0 ? (
            <pre data-prefix="$">
              <code>
                Welcome! Type <b>/list</b> to see topics.
              </code>
            </pre>
          ) : (
            output.map((line, idx) => (
        ...
        <p className="text-sm mt-2 text-gray-500">
          Use <code className="bg-gray-100 px-1 rounded">/list</code> to view
          topics, or{" "}
          <code className="bg-gray-100 px-1 rounded">/select/topic</code> to
          navigate.
        </p>
      </div>
    </div>
